...
import { Handler } from '../../../../core/models/handler.js';
import { ErrorStickerMessage } from '../messages/ErrorMessage.js';
...
// export class handlerStickerComand {
...
async function StickerCommand({msg,client}){
  ...
  //   }
    ...
    if (!mediaMsg|| mediaMsg === null) {
      await client.send.reply(msg,ErrorStickerMessage.respondMsg())
      return;
    }
    ...
    await client.send.reply(msg, '⚠️ Error al crear el sticker.');
        ...
        if (!fs.existsSync(tempOutput)) {
          throw new Error(ErrorStickerMessage.NotFoundFile());
        }
